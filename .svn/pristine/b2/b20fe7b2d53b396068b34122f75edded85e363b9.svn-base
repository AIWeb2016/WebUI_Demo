...
$.fn.calendar.defaults={width:180,height:219,fit:false,border:true,firstDay:0,weeks:["日","一","二","三","四","五","六"],months:["01月","02月","03月","04月","05月","06月","07月","08月","09月","10月","11月","12月"],year:new Date().getFullYear(),month:new Date().getMonth()+1,current:(function(){
var d=new Date();
...
$.fn.datebox.defaults=$.extend({},$.fn.combo.defaults,{panelWidth:246,panelHeight:280,sharedCalendar:null,keyHandler:{up:function(e){
},down:function(e){
},left:function(e){
},right:function(e){
},enter:function(e){
...
(function($) {
	...
	AIStep.prototype = {
		...
		init: function(params) {
			...
			this.stepNum = settings.step;
			this.stepTotal = settings.item.length;
			this.visited = settings.visited;
			this.afterStep = settings.afterStep;
			var itemHTML = '';
			$.each(params.item, function(btnIdx, obj) {

				itemHTML += '<li class="ui-step-item">' + '<span class="ui-step-text">' + obj.text + '</span></li>';
			...
			this.renderStep();
			var me = this;
			this.el.find(".ui-step").delegate("li.ui-step-visited", "click", function() {
				debugger;
				me.stepNum = $(this).index() + 1;
				me.renderStep();
				me.afterStep(me);
			});
		},
		renderStep: function() {
			for (var i = 0; i < this.stepTotal; i++) {
				if (i < this.stepNum - 1) {
					if (this.visited) {
						this.el.find(".ui-step").find("li").eq(i).removeClass().addClass("ui-step-item").addClass("ui-step-visited");
					} else {
						this.el.find(".ui-step").find("li").eq(i).removeClass().addClass("ui-step-item");
					}
				} else if (i > this.stepNum - 1) {
					this.el.find(".ui-step").find("li").eq(i).removeClass().addClass("ui-step-item");
				} else {
					this.el.find(".ui-step").find("li").eq(i).removeClass().addClass("ui-step-item").addClass("ui-step-active");
				}
			}
		},
		nextStep: function() {
			this.stepNum++;
			if (this.stepNum > this.stepTotal) {
				this.stepNum--;
				return false;
			} else {
				this.renderStep();
	...
	$.fn.AIStep = function(params) {
		return new AIStep(this, params);
	}
